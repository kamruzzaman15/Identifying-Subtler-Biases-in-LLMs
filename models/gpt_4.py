...
for index, data in framing_data[:25].iterrows():
  ...
  try:
    ...
    # Sometimes rate limit exceeds for per minute request quota. We need to wait a minute to continue execution
    time.sleep(60)
...
# GPT-4 model responds with a value that matches the three provided options
df1 = pd.DataFrame(framing_data_valid_result)
df1.to_csv('outputs/gpt_4_valid.csv', index = False, encoding='utf-8')

# GPT-4 model responds with a value that does not match the three provided options
df2 = pd.DataFrame(framing_data_invalid_result)
df2.to_csv('outputs/gpt_4_invalid.csv', index = False, encoding='utf-8')
